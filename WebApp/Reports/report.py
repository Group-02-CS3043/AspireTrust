...
from Configurations.configurations import valid_manager
...
def get_withdraws(min_val , max_val,start_date ,end_date , branch_id):
    
        connector = Connector()
        try:
            with connector:
    
                connector.cursor.execute(GET_WITHDRAWS,(min_val,max_val,start_date ,end_date,branch_id,))
                return connector.cursor.fetchall()
        except Exception as e:
            print("Error in get_withdraws",e)
            return None
...
@report_app.route('/all_withdraws',methods = DEFUALT_SUBMISSION_METHODS,endpoint='all_withdraws')
@valid_manager
def all_withdraws_branchwise():
    if request.method == 'GET':
        context = {}
        branch_id = get_branch_id(session['user_id'])['branch_id']
        start_date = '2019-01-01'
        ...
        max_value = 10000
        context['transactions'] = get_withdraws(min_value , max_value , start_date,end_date,branch_id)
        context['start_date'] = start_date
        context['end_date'] = end_date
        context['min_value'] = min_value
        context['max_value'] =max_value
        print("context",context)
        return render_template('reports/allwithdraws.html',context = context)
    elif request.method == 'POST':
        print(request.form)
        context = {}
        branch_id = get_branch_id(session['user_id'])['branch_id']
